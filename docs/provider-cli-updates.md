...
- **Runtime baseline:** Super Synara's native transcript integration requires Command Code v1.0.0 or newer and fails closed when `--version` reports an older or unparseable version. The current npm package declares Node.js 22 or newer ([npm `latest` metadata](https://registry.npmjs.org/command-code/latest)). Install or upgrade the vendor package with `npm i -g command-code@latest` ([Command Code v1 overview](https://commandcode.ai/docs/whats-new), [Command Code quickstart](https://commandcode.ai/docs/quickstart)).
...
- **Headless v1 protocol:** each turn sends the prompt through stdin and launches print mode with real flags, `--output-format json`, a ten-turn limit, and automatic updates disabled. Standard output is parsed incrementally as NDJSON: supported text, reasoning, tool, session, usage, and terminal result records become native transcript events; unknown event types are ignored; malformed, truncated, duplicate-terminal, post-terminal, session-inconsistent, or over-budget streams fail the turn. Usage is normalized from Command Code's per-run totals into Synara's cumulative per-thread counter without double-counting cache details. The terminal result is authoritative, while its `finalText` is used only when no text delta was streamed. Structured `error` and `max_turns` outcomes, documented exit codes 3–10, and exit 130 are surfaced without displaying protocol JSON as assistant text ([Command Code headless mode](https://commandcode.ai/docs/core-concepts/headless)).
- **State migration boundary:** Command Code v1 automatically migrates its documented legacy keybinding settings, pasted-text collapsing setting, and per-project taste state on first save. The upstream v1 notes separately describe pre-v1 sessions as backward compatible; they do not describe those sessions as part of the settings migration. Super Synara performs neither migration and does not read or rewrite `~/.commandcode` state ([Command Code v1 overview](https://commandcode.ai/docs/whats-new)).
- **macOS and Linux:** the vendor supports both platforms. Super Synara resolves the selected alias normally, launches it without a shell, and owns the detached process group so interruption and shutdown cover descendants. WSL is supported by the vendor as a Linux environment but remains a separate installation; native Super Synara does not implicitly bridge to a WSL-only CLI.
- **Native Windows:** the dedicated vendor guide still labels native support alpha and recommends WSL, despite the broader v1 announcement describing full Windows support. Super Synara therefore keeps the conservative support wording. Use `cmdc` rather than bare `cmd`, because `cmd` is also the Windows command processor. Command Code's native shell-command tool requires PowerShell 7 or newer. Runtime discovery accepts the package's npm shims but rejects `cmd.exe`/`cmd.com`, preserves verbatim `.cmd` arguments, hides child windows, and contains the provider tree in a Windows Job Object. The v1 NDJSON reader accepts CRLF and arbitrary UTF-8 chunk boundaries ([Command Code Windows guide](https://commandcode.ai/docs/troubleshooting/windows)).
