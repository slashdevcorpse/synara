...
- **Windows notes:** Keep provider runtimes quiesced and the maintenance gate closed across both vendor commands and the final version probe. Never replace Droid's executable or edit Factory's pending-update files directly; both phases must use the exact resolved executable and fail closed on target drift or unproven process exit.

Factory's [public CLI reference](https://docs.factory.ai/reference/cli-reference) documents both `droid update` and `droid update --check`. Source-specific package-manager updates remain the conservative choice when package ownership is known.
