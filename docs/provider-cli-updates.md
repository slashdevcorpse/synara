...
- **Vendor-supported channels:** Factory's native shell or PowerShell installer, Homebrew cask, and npm package `droid` ([Droid quickstart](https://docs.factory.ai/cli/getting-started/quickstart), [Factory repository](https://github.com/Factory-AI/factory#installation)). A legacy npm identity, [`@factory/cli`](https://registry.npmjs.org/%40factory%2Fcli/latest), also remains observable in the registry but is not interchangeable with `droid`.
- **Implemented in this build:** exact npm package `droid`, with an allowed `droid` manifest bin and accepted local `latest` evidence: `npm install -g droid@latest`, pinned to the detected prefix.
- **Version probe:** `droid --version`.
- **Vendor-supported but manual-only/planned:** the native `droid update` self-updater, Homebrew, the legacy `@factory/cli` package, custom paths, and ambiguous wrappers until this build can prove their separate package identities, roots, and same-channel targets. A Homebrew cask must be updated with its owning Homebrew installation; a legacy package must remain `@factory/cli`. Never silently migrate either one to npm `droid`.
- **Windows notes:** Factory documents a native PowerShell installer but does not document its replacement guarantees. Apply the normal quiescence and sharing-violation handling.

Factory's [public CLI reference](https://docs.factory.ai/reference/cli-reference) documents the bare `droid update` command. **Artifact-verified, 2026-07-20:** the current vendor-published [`droid` package](https://registry.npmjs.org/droid/latest) additionally exposed `droid update --check` and version-targeted update options. Those extra flags are artifact evidence, not a public compatibility promise; source-specific package-manager updates remain the conservative choice when ownership is known.
