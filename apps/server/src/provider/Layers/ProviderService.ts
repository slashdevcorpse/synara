...
import { type ProviderServiceError, ProviderValidationError } from "../Errors.ts";
...
export interface ProviderServiceLiveOptions {
  ...
  /** Shared admission gate used to exclude provider process work during CLI maintenance. */
...
type PrepareForMaintenance = NonNullable<ProviderServiceShape["prepareForMaintenance"]>;
type StopRuntimeSessionIfIdle = NonNullable<ProviderServiceShape["stopRuntimeSessionIfIdle"]>;
const RUNTIME_IDLE_STOP_CANCELLED = Symbol("provider.runtimeIdleStopCancelled");
...
function isTransientMaintenanceBusy(error: unknown): boolean {
  return (
    error instanceof ProviderValidationError &&
    error.cause instanceof ProviderMaintenanceBusyError &&
    error.cause.latchedReason === null
  );
}
...
function runtimeEventClearsActiveTurn(event: ProviderRuntimeEvent): boolean {
...
const makeProviderService = (options?: ProviderServiceLiveOptions) =>
  Effect.gen(function* () {
    ...
    const withProviderOperation = <A, E, R>(input: {
    ...
    for (const binding of persistedBindings) {
      ...
      const adapterExit = yield* Effect.exit(registry.getByProvider(binding.provider));
      if (Exit.isFailure(adapterExit)) {
        yield* Effect.logWarning(
          "provider startup recovery could not resolve persisted session adapter",
          {
      ...
      if (Exit.isFailure(writeExit)) {
        yield* Effect.logWarning(
          "provider startup recovery failed to normalize crash-orphaned session",
          {
            threadId: binding.threadId,
            provider: binding.provider,
            persistedStatus: binding.status,
            cause: Cause.pretty(writeExit.cause),
          },
        );
        continue;
      }

      yield* Effect.logInfo("provider startup recovery normalized crash-orphaned session", {
        threadId: binding.threadId,
        provider: binding.provider,
        persistedStatus: binding.status,
        recoveredAt,
    ...
    const idleSensitiveWorkCounts = new Map<ThreadId, number>();
    ...
    const acquireIdleSensitiveWork = (threadId: ThreadId): Effect.Effect<void> =>
      Effect.suspend(() => {
        const inFlightStop = runtimeIdleStopsInFlight.get(threadId);
        if (inFlightStop) {
          return Effect.promise(() => inFlightStop).pipe(
            Effect.andThen(acquireIdleSensitiveWork(threadId)),
          );
        }

        // Claim ownership in the same synchronous section that observes no
    ...
    const releaseIdleSensitiveWork = (threadId: ThreadId): void => {
      const remaining = (idleSensitiveWorkCounts.get(threadId) ?? 1) - 1;
      if (remaining > 0) {
        idleSensitiveWorkCounts.set(threadId, remaining);
      } else {
        idleSensitiveWorkCounts.delete(threadId);
      }
    };
        ...
        return Effect.acquireUseRelease(
          ...
          (_acquired, exit) =>
            Effect.sync(() => {
              ...
              if (
                (Exit.isSuccess(exit) && options?.scheduleIdleStopOnSuccess === true) ||
                (Exit.isFailure(exit) && displacedIdleStop)
              ) {
                scheduleRuntimeIdleStop(threadId);
    ...
    const persistRuntimeEvent = (event: ProviderRuntimeEvent): Effect.Effect<void> =>
      options?.persistRuntimeEvent ? options.persistRuntimeEvent(event) : Effect.void;
    const fanOutRuntimeEvent = (event: ProviderRuntimeEvent): Effect.Effect<void> =>
    ...
    const hasTrackedRuntimeWork = (threadId: ThreadId): boolean => {
      const dispatchState = dispatchStateByThread.get(threadId);
      return (
        (idleSensitiveWorkCounts.get(threadId) ?? 0) > 0 ||
        (dispatchState?.inFlightGenerations.size ?? 0) > 0 ||
        (dispatchState?.outstandingTurnIds.size ?? 0) > 0 ||
          ...
          const activeTurnId =
            event.type === "turn.started"
              ...
              : event.type === "thread.state.changed" && event.payload.state === "compacted"
                ...
                : runtimeEventClearsActiveTurn(event)
                  ? null
                  : currentActiveTurnId;
      ...
      Effect.uninterruptible(
        Effect.gen(function* () {
          ...
          yield* Effect.sync(() => {
            ...
            Effect.andThen(
              persistBeforeBinding
                ? persistEvent.pipe(Effect.andThen(updateBinding))
                : updateBinding.pipe(Effect.andThen(persistEvent)),
            ),
            Effect.andThen(fanOutRuntimeEvent(canonicalEvent)),
          );
        }),
      );
    ...
    const recoverSessionForThread = (input: {
      ...
      lifecycle.run(input.binding.threadId, (lease) =>
        Effect.gen(function* () {
          ...
          const resumed = yield* withProviderOperation({
        ...
        return yield* runIdleSensitiveProviderWork(
          ...
          lifecycle.run(threadId, (lease) =>
            Effect.gen(function* () {
              ...
              const startWithReplacementAdmission = withProviderOperation({
                provider: adapter.provider,
                operation: "ProviderService.startSession",
                run: startAndPersistReplacement,
              });

              if (!persistedBinding || persistedBinding.provider === input.provider) {
                return yield* startWithReplacementAdmission;
              }

              const previousAdapter = yield* registry.getByProvider(persistedBinding.provider);
              const switchTransaction = Effect.gen(function* () {
              ...
              // Acquire both provider admissions before inspecting or stopping the old
              // runtime, then hold both through replacement and any rollback.
              return yield* withProviderOperation({
                provider: previousAdapter.provider,
                operation: "ProviderService.startSession.switchPreviousProvider",
                run: withProviderOperation({
                  provider: adapter.provider,
                  operation: "ProviderService.startSession.switchReplacementProvider",
                  run: switchTransaction,
                }),
              });
            }),
          ),
        ...
        return yield* runIdleSensitiveProviderWork(
          ...
          runIdleSensitiveProviderWork(
            ...
            Effect.gen(function* () {
              ...
              const forked = yield* withProviderOperation({
                provider: adapter.provider,
                operation: "ProviderService.forkThread",
                run: adapter
                  .forkThread({
      ...
      Effect.gen(function* () {
        ...
        return yield* runTurnDispatch(input.threadId, (generation) =>
          Effect.gen(function* () {
            ...
            const turn = yield* withProviderOperation({
              provider: routed.adapter.provider,
              operation: "ProviderService.sendTurn",
              run: routed.adapter.sendTurn(input),
            });
        ...
        return yield* runTurnDispatch(input.threadId, (generation) =>
          Effect.gen(function* () {
            ...
            const turn = yield* withProviderOperation({
              provider: routed.adapter.provider,
              operation: "ProviderService.steerTurn",
              run: routed.adapter.steerTurn(input),
            });
            const persistenceInput: StartedTurnPersistenceInput = {
              threadId: input.threadId,
              provider: routed.adapter.provider,
        ...
        return yield* runTurnDispatch(input.threadId, (generation) =>
          Effect.gen(function* () {
            ...
            const turn = yield* withProviderOperation({
              provider: routed.adapter.provider,
              operation: "ProviderService.startReview",
              run: routed.adapter.startReview(input),
            });
            const persistenceInput: StartedTurnPersistenceInput = {
              threadId: input.threadId,
              provider: routed.adapter.provider,
          ...
          runIdleSensitiveProviderWork(
            ...
            lifecycle.runCurrent(input.threadId, () =>
              Effect.gen(function* () {
                ...
                yield* withProviderOperation({
                  provider: routed.adapter.provider,
                  operation: "ProviderService.backgroundTask",
                  run: routed.adapter.backgroundTask(input.threadId, input.toolUseId),
                });
            ...
            lifecycle.runCurrent(input.threadId, () =>
              Effect.gen(function* () {
                ...
                yield* withProviderOperation({
                  provider: routed.adapter.provider,
                  operation: "ProviderService.steerSubagent",
                  run: routed.adapter.steerSubagent(input.threadId, input.providerThreadId, {
                    input: input.input ?? "",
    ...
    const respondToInteraction = (response: InteractionResponse) => {
      ...
      return runIdleSensitiveProviderWork(
        input.threadId,
        lifecycle.runCurrent(input.threadId, (currentGeneration) =>
          Effect.gen(function* () {
            const routed = yield* resolveRoutableSession({
              threadId: input.threadId,
              operation,
            ...
            if (response.kind === "approval") {
              yield* withProviderOperation({
                provider: routed.adapter.provider,
                operation,
                run: routed.adapter.respondToRequest(
                  input.threadId,
                  input.requestId,
            ...
            yield* withProviderOperation({
              provider: routed.adapter.provider,
              operation,
              run: routed.adapter.respondToUserInput(
                input.threadId,
                input.requestId,
    ...
    const prepareForMaintenance: PrepareForMaintenance = (input) =>
      Effect.gen(function* () {
        const operation = "ProviderService.prepareForMaintenance";
        const adapter = yield* registry.getByProvider(input.provider);
        const initialSessions = (yield* adapter.listSessions()).filter(
          (session) => session.provider === input.provider && session.status !== "closed",
        );

        if (!input.stopIdleSessions && initialSessions.length > 0) {
          return yield* toValidationError(
            operation,
            `Cannot update '${input.provider}' while ${initialSessions.length} runtime${initialSessions.length === 1 ? " is" : "s are"} open because process ownership cannot be proven safely. Close the affected sessions and retry.`,
          );
        }
        ...
        const snapshots = yield* Effect.forEach(
          ...
          (session) =>
            Effect.gen(function* () {
              ...
              if (!binding || binding.provider !== input.provider) {
                return yield* toValidationError(
                  operation,
                  `Cannot update '${input.provider}' because runtime '${session.threadId}' does not have a matching persisted provider binding.`,
                );
              }
              const hasActiveWork =
                ...
                hasTrackedRuntimeWork(session.threadId);
              if (hasActiveWork) {
                return yield* toValidationError(
                  operation,
                  `Cannot update '${input.provider}' while runtime '${session.threadId}' has active work. Wait for turns and background tasks to finish, then retry.`,
                );
              }
              const resumeCursor = hasResumeCursor(session.resumeCursor)
                ? session.resumeCursor
                : binding.resumeCursor;
              if (!hasResumeCursor(resumeCursor)) {
                return yield* toValidationError(
                  operation,
                  `Cannot update '${input.provider}' because runtime '${session.threadId}' has no verified resume cursor. Close the session explicitly and retry.`,
                );
              }
              return {
        ...
        for (const snapshot of snapshots) {
          clearRuntimeIdleTimer(snapshot.threadId);
          yield* waitForRuntimeIdleStop(snapshot.threadId);
        }

        type MaintenanceLease = {
          readonly generation: string;
          readonly snapshot: (typeof snapshots)[number];
        };
        const acquireLifecycleLocks = (
          index: number,
          leases: ReadonlyArray<MaintenanceLease>,
        ): Effect.Effect<ReadonlyArray<ThreadId>, ProviderServiceError> => {
          const snapshot = snapshots[index];
          if (!snapshot) {
            return Effect.gen(function* () {
              const currentSessions = (yield* adapter.listSessions()).filter(
                (session) => session.provider === input.provider && session.status !== "closed",
              );
              const sessionsByThreadId = new Map(
                currentSessions.map((session) => [session.threadId, session] as const),
              );
              const validated = yield* Effect.forEach(
                leases,
                ({ generation, snapshot: lockedSnapshot }) =>
                  Effect.gen(function* () {
                    const binding = Option.getOrUndefined(
                      yield* directory.getBinding(lockedSnapshot.threadId),
                    );
                    const session = sessionsByThreadId.get(lockedSnapshot.threadId);
                    if (
                      !binding ||
                      binding.provider !== input.provider ||
                      binding.lifecycleGeneration !== lockedSnapshot.lifecycleGeneration ||
                      !session
                    ) {
                      return yield* toValidationError(
                        operation,
                        `Cannot update '${input.provider}' because runtime '${lockedSnapshot.threadId}' changed during maintenance preparation. Retry the update.`,
                      );
                    }
                    const hasActiveWork =
                      session.status === "connecting" ||
                    ...
                    if (!hasResumeCursor(resumeCursor)) {
                      return yield* toValidationError(
                        operation,
                        `Cannot update '${input.provider}' because runtime '${session.threadId}' no longer has a verified resume cursor. Close the session explicitly and retry.`,
                      );
                    }
                    return { binding, generation, resumeCursor, session };
                  }),
                { concurrency: 1 },
              );
              ...
              yield* Effect.forEach(
                validated,
                ({ binding, generation, resumeCursor, session }) =>
                  Effect.gen(function* () {
                    yield* adapter.stopSession(session.threadId);
                    liveRuntimeTaskIds.delete(session.threadId);
                    yield* withBindingWriteLock(
                      session.threadId,
                      directory.upsert({
                        threadId: session.threadId,
                        ...
                        runtimePayload: {
                          ...runtimePayloadRecord(binding.runtimePayload),
                          activeTurnId: null,
                          lastRuntimeEvent: "provider.prepareForMaintenance",
                          lastRuntimeEventAt: new Date().toISOString(),
                          lifecycleGeneration: generation,
                        },
                      }),
              ...
              const remaining = (yield* adapter.listSessions()).filter(
                (session) => session.provider === input.provider && session.status !== "closed",
              );
              if (remaining.length > 0) {
                return yield* toValidationError(
                  operation,
                  `Cannot update '${input.provider}' because ${remaining.length} runtime${remaining.length === 1 ? " is" : "s are"} still open after shutdown.`,
                );
              }
              return validated.map(({ session }) => session.threadId);
            });
          }

          return lifecycle.run(snapshot.threadId, (lease) =>
            acquireLifecycleLocks(index + 1, [
              ...leases,
              { generation: lease.generation, snapshot },
            ]),
          );
        };

        return yield* acquireLifecycleLocks(0, []);
      });
    ...
    const isExplicitlyIdleReadySession = (
      session: ProviderSession,
      binding: ProviderRuntimeBinding,
    ): boolean => {
    ...
    const cancelRuntimeIdleStop = (): Effect.Effect<never, typeof RUNTIME_IDLE_STOP_CANCELLED> =>
      Effect.fail(RUNTIME_IDLE_STOP_CANCELLED);

    const stopRuntimeSessionIfIdleInternal = (
      threadId: ThreadId,
      expectedIdleGeneration: symbol,
    ) => {
      const isEligibleBeforeStop = (binding: ProviderRuntimeBinding): boolean => {
        const activeTurnId = runtimePayloadRecord(binding.runtimePayload).activeTurnId;
        return (
          isRuntimeIdleGenerationCurrent(threadId, expectedIdleGeneration) &&
          !hasTrackedRuntimeWork(threadId) &&
          (activeTurnId === null || activeTurnId === undefined)
        );
      };

      return lifecycle
        .runConditional(threadId, (lease) =>
          Effect.gen(function* () {
            if (!isRuntimeIdleGenerationCurrent(threadId, expectedIdleGeneration)) {
              return yield* cancelRuntimeIdleStop();
            }

            const binding = Option.getOrUndefined(yield* directory.getBinding(threadId));
            if (!binding || !isEligibleBeforeStop(binding)) {
              return yield* cancelRuntimeIdleStop();
            }

            const originalProvider = binding.provider;
            const originalLifecycleGeneration = binding.lifecycleGeneration;
            if (
              originalLifecycleGeneration !== undefined &&
              lease.currentGeneration !== originalLifecycleGeneration
            ) {
              return yield* cancelRuntimeIdleStop();
            }
            const adapter = yield* registry.getByProvider(binding.provider);
            const [hasSession, sessions] = yield* Effect.all([
              adapter.hasSession(threadId),
              adapter.listSessions(),
            ]);
            const matchingSessions = sessions.filter((session) => session.threadId === threadId);
            const liveSession = matchingSessions[0];

            // Treat contradictory adapter snapshots as busy. Teardown is safe
            // only when both adapter APIs agree on exact ownership.
            if (matchingSessions.length > 1 || hasSession !== (matchingSessions.length === 1)) {
              return yield* cancelRuntimeIdleStop();
            }

            if (
              liveSession &&
              (!isExplicitlyIdleReadySession(liveSession, binding) ||
                liveSession.activeTurnId != null ||
                (!hasResumeCursor(liveSession.resumeCursor) &&
            ...
            yield* withBindingWriteLock(
              threadId,
              Effect.gen(function* () {
                // Runtime events share this binding lock. Re-read under it so
                // an old-generation task/turn that arrived during adapter
                // inspection either invalidates cleanup now or becomes stale
                // only after the lifecycle generation advances below.
                const currentBinding = Option.getOrUndefined(yield* directory.getBinding(threadId));
                if (
                  !lease.isCurrent() ||
                  !currentBinding ||
                  currentBinding.provider !== originalProvider ||
                  currentBinding.lifecycleGeneration !== originalLifecycleGeneration ||
                  !isEligibleBeforeStop(currentBinding)
                ) {
                  return yield* cancelRuntimeIdleStop();
                }

                const freshestResumeCursor =
                  liveSession?.resumeCursor ?? currentBinding.resumeCursor;
                yield* Effect.uninterruptible(
                  Effect.gen(function* () {
                    if (liveSession) {
                      // This is the destructive boundary. New idle-sensitive
                      // work waits on the registered stop. Once teardown starts,
                      // always finish the durable stopped write even if a late
                      // event invalidates the idle generation.
                      yield* withProviderOperation({
                        provider: adapter.provider,
                        operation: "ProviderService.stopRuntimeSessionIfIdle",
                        run: Effect.suspend(() => adapter.stopSession(threadId)),
                      });
                    }

                    liveRuntimeTaskIds.delete(threadId);
                    const stoppedGeneration = lease.advance();
                    yield* directory.upsert({
                      threadId,
                      provider: currentBinding.provider,
                      ...(currentBinding.adapterKey !== undefined
                        ? { adapterKey: currentBinding.adapterKey }
                        : {}),
                      ...(currentBinding.runtimeMode !== undefined
                        ? { runtimeMode: currentBinding.runtimeMode }
                        : {}),
                      status: "stopped",
                      lifecycleGeneration: stoppedGeneration,
                      resumeCursor: freshestResumeCursor,
                      runtimePayload: {
                        ...runtimePayloadRecord(currentBinding.runtimePayload),
                        activeTurnId: null,
                        lastRuntimeEvent: "provider.stopRuntimeSession",
                        lastRuntimeEventAt: new Date().toISOString(),
                        lifecycleGeneration: stoppedGeneration,
        ...
        .pipe(
          Effect.catch((error) =>
            error === RUNTIME_IDLE_STOP_CANCELLED ? Effect.void : Effect.fail(error),
          ),
          Effect.ensuring(
            Effect.sync(() => retireRuntimeIdleGeneration(threadId, expectedIdleGeneration)),
          ),
        );
    };

    const runRegisteredRuntimeIdleStop = (
      threadId: ThreadId,
      scheduledGeneration?: symbol,
    ): Effect.Effect<void, ProviderServiceError> =>
      Effect.suspend(() => {
        const existingStop = runtimeIdleStopsInFlight.get(threadId);
        if (existingStop) {
          return Effect.promise(() => existingStop);
        }

        const generation = scheduledGeneration ?? invalidateRuntimeIdleGeneration(threadId);
        const workOwnedRuntimeAtRegistration = hasTrackedRuntimeWork(threadId);
        let releaseStop!: () => void;
        const stopCompletion = new Promise<void>((resolve) => {
          releaseStop = resolve;
        });
        runtimeIdleStopsInFlight.set(threadId, stopCompletion);

        const stop = workOwnedRuntimeAtRegistration
          ? Effect.sync(() => retireRuntimeIdleGeneration(threadId, generation))
          : stopRuntimeSessionIfIdleInternal(threadId, generation);
        return stop.pipe(
          Effect.ensuring(
            Effect.sync(() => {
              if (runtimeIdleStopsInFlight.get(threadId) === stopCompletion) {
    ...
    const stopRuntimeSessionIfIdle: StopRuntimeSessionIfIdle = (rawInput) =>
      Effect.gen(function* () {
        const input = yield* decodeInputOrValidationError({
          operation: "ProviderService.stopRuntimeSessionIfIdle",
          schema: ProviderStopSessionInput,
        ...
        yield* runRegisteredRuntimeIdleStop(input.threadId);
      });

    stopIdleRuntimeSession = (threadId, generation) => {
      void Effect.runPromise(
        runRegisteredRuntimeIdleStop(threadId, generation).pipe(
          Effect.catch((error) =>
            isTransientMaintenanceBusy(error)
        ...
        yield* runIdleSensitiveProviderWork(
          ...
          Effect.gen(function* () {
            ...
            if (routed.adapter.capabilities.conversationRollback === "restart-session") {
              ...
              yield* withProviderOperation({
                provider: active.adapter.provider,
                operation: "ProviderService.rollbackConversation",
                run: active.adapter.rollbackThread(input.threadId, input.numTurns),
              });
            }
        ...
        yield* runIdleSensitiveProviderWork(
          ...
          Effect.gen(function* () {
            ...
            yield* withProviderOperation({
              provider: routed.adapter.provider,
              operation: "ProviderService.compactThread",
              run: routed.adapter.compactThread(input.threadId),
            });
            const binding = Option.getOrUndefined(yield* directory.getBinding(input.threadId));
            if (binding) {
              yield* directory.upsert({
