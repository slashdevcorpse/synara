...
routing.layer("ProviderServiceLive routing", (it) => {
  ...
  it.effect("requires the source lifecycle generation for modern Gemini approvals", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService;
      const directory = yield* ProviderSessionDirectory;
      const threadId = asThreadId("thread-gemini-interaction-generation");
      ...
      const responseCallCount = routing.gemini.respondToRequest.mock.calls.length;
      const missingGeneration = yield* Effect.result(
        provider.respondToRequest({
          threadId,
          requestId: asRequestId("gemini-approval-without-generation"),
          decision: "accept",
        }),
      );
      assertFailure(
        ...
        new ProviderValidationError({
          operation: "ProviderService.respondToRequest",
          issue:
            "Cannot respond to request 'gemini-approval-without-generation' without its provider lifecycle generation.",
        }),
      );
      assert.equal(routing.gemini.respondToRequest.mock.calls.length, responseCallCount);

      yield* provider.respondToRequest({
        threadId,
        requestId: asRequestId("gemini-approval-current-generation"),
        lifecycleGeneration,
        decision: "accept",
      });
      assert.equal(routing.gemini.respondToRequest.mock.calls.length, responseCallCount + 1);

      yield* provider.stopSession({ threadId });
      routing.gemini.startSession.mockClear();
      routing.gemini.respondToRequest.mockClear();
      routing.gemini.stopSession.mockClear();
    }),
  );
