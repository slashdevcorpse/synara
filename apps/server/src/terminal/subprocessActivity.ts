...
type SubprocessChildrenMap = ReadonlyMap<
  number,
...
function conservativeBusySubprocessActivity(): TerminalSubprocessActivity {
  return {
    cliKind: null,
...
export function inspectSubprocessActivity(
  parentPid: number,
  childrenByParentPid: SubprocessChildrenMap,
): TerminalSubprocessActivity {
...
function inspectSubprocessActivityFromSnapshot(
  ...
  for (const child of children) {
    ...
    if (visitedPids.has(child.pid)) {
      // A cyclic snapshot is malformed. Treat the uninspectable branch as busy
      // instead of letting a shell-only cycle become a false idle result.
      nestedActivity = conservativeBusySubprocessActivity();
    } else if (visitedPids.size >= POSIX_SUBPROCESS_TREE_WALK_MAX_VISITED) {
      // A malformed or unexpectedly deep snapshot must never recurse forever
      // or turn an uninspected process tree into a false idle result.
      nestedActivity = conservativeBusySubprocessActivity();
    } else {
...
async function checkPosixSubprocessActivityByTreeWalk(
  ...
  const inspectPid = async (parentPid: number): Promise<TerminalSubprocessActivity> => {
    if (visited >= POSIX_SUBPROCESS_TREE_WALK_MAX_VISITED) {
      return conservativeBusySubprocessActivity();
    }
