...
describe("inspectSubprocessActivity", () => {
  ...
  it("fails closed when a malformed snapshot contains only a shell cycle", () => {
    const map = buildChildrenMap([
      { ppid: 100, pid: 200, command: "zsh" },
      { ppid: 200, pid: 100, command: "bash" },
    ]);
  ...
  it("fails closed when a snapshot exceeds the traversal budget", () => {
    ...
    expect(inspectSubprocessActivity(100, buildChildrenMap(entries))).toEqual({
      cliKind: null,
      hasNonProviderSubprocess: true,
      hasProviderDescendant: false,
      hasRunningSubprocess: true,
