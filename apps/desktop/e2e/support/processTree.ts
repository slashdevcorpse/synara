...
async function forceTerminateChildHandle(
  child: ChildProcess,
  expectedProcess: TrackedProcess,
  dependencies: ProcessTreeDependencies,
): Promise<void> {
  if (childHasExited(child)) return;
  if (child.pid !== expectedProcess.pid) {
    throw new Error(
      `Electron child handle pid ${String(child.pid)} did not match tracked root pid ${expectedProcess.pid}.`,
    );
  }

  const verificationErrors: unknown[] = [];
  const verifiedProcesses = readMatchingTrackedProcesses(
    [expectedProcess],
    verificationErrors,
    "immediately before child-handle forced termination",
    dependencies,
  );
  if (verificationErrors.length === 1) throw verificationErrors[0];
  if (verificationErrors.length > 1) {
    throw new AggregateError(
      verificationErrors,
      "Electron child-handle identity verification failed.",
    );
  }
  if (verifiedProcesses.length !== 1) return;
...
function signalTrackedProcesses(
  ...
  const signaledProcesses: TrackedProcess[] = [];
  for (const trackedProcess of trackedProcesses) {
    ...
    if (matchingProcesses.length !== 1) continue;
    try {
      dependencies.signalProcess(trackedProcess.pid, signal);
      signaledProcesses.push(trackedProcess);
    } catch (error) {
