...
function ensureUniqueManagedHeaders(document, headers, configPath) {
  for (const header of headers) {
    ...
    if (count > 1) {
      fail(
        `Existing Codex config has duplicate section [${header}] in ${configPath}; refusing to modify it.`,
      );
    }
  }
}
...
function installClaudeMcp(configPath, projectRoot, serverName, commandName) {
  ...
  const legacyServerName = "wandy";
  const data = readJSONObjectConfig(configPath, `Claude config ${configPath}`);
  const projects = ensureObjectField(
    data,
    "projects",
    'Existing Claude config has non-object "projects"; refusing to modify it.',
  );
  ...
  if (targetMatches && !legacyMatches) {
    process.stdout.write(
      `Claude MCP server "${serverName}" is already installed for ${projectRoot} in ${configPath}.\n`,
    );
    return;
  }
  ...
  if (targetMatches && legacyMatches) {
    process.stdout.write(
      `Claude MCP server "${serverName}" was already installed for ${projectRoot}; removed legacy alias "${legacyServerName}" from ${configPath}.\n`,
    );
  } else {
    process.stdout.write(
      `Installed Claude MCP server "${serverName}" for ${projectRoot} into ${configPath}.\n`,
    );
  }
}
...
function installGeminiMcp(configPath, serverName, commandName) {
  ...
  if (targetMatches && !legacyMatches) {
    process.stdout.write(
      `Gemini MCP server "${serverName}" is already installed in ${configPath}.\n`,
    );
    return;
  }
  ...
  if (targetMatches && legacyMatches) {
    process.stdout.write(
      `Gemini MCP server "${serverName}" was already installed; removed legacy alias "${legacyServerName}" from ${configPath}.\n`,
    );
  } else {
...
function installOpencodeMcp(primaryConfigPath, secondaryConfigPath, serverName, commandName) {
  ...
  if (
    ...
    extraAliases.length === 1 &&
    targetMatches[0] === extraAliases[0]
  ) {
    process.stdout.write(
      `opencode MCP server "${serverName}" is already installed in ${targetMatches[0]}.\n`,
    );
    return;
  }
  ...
  process.stdout.write(
    `Installed opencode MCP server "${serverName}" into ${primaryConfigPath}.\n`,
  );
}

function installCodexMcp(configPath, serverName, commandName) {
  const desiredBody = [`command = ${JSON.stringify(commandName)}`, 'args = ["mcp"]'].join("\n");
  const targetHeader = `mcp_servers."${serverName}"`;
  const targetEnvHeader = `mcp_servers."${serverName}".env`;
  const legacyServerName = "wandy";
  const legacyHeader = `mcp_servers."${legacyServerName}"`;
  const legacyEnvHeader = `mcp_servers."${legacyServerName}".env`;
  const text = readTextIfExists(configPath);
  const document = splitTomlSections(text);

  ensureUniqueManagedHeaders(
    document,
    [targetHeader, targetEnvHeader, legacyHeader, legacyEnvHeader],
    configPath,
  );
  ...
  const targetEnvSection = document.sections.find((section) => section.header === targetEnvHeader);
  const legacyEnvSection = document.sections.find((section) => section.header === legacyEnvHeader);
  const desiredCanonical = canonicalSectionBody(desiredBody.split("\n"));
  const targetMatches = targetSection
    ? canonicalSectionBody([
        ...targetSection.bodyLines,
        ...(targetEnvSection
          ? [`[${targetEnvSection.header}]`, ...targetEnvSection.bodyLines]
          : []),
      ]) === desiredCanonical
    : false;
  const legacyMatches =
  ...
  if (targetMatches && !legacyMatches) {
    process.stdout.write(
      `Codex MCP server "${serverName}" is already installed in ${configPath}.\n`,
    );
    return;
  }
  ...
  if (targetMatches && legacyMatches) {
    process.stdout.write(
      `Codex MCP server "${serverName}" was already installed; removed legacy alias "${legacyServerName}" from ${configPath}.\n`,
    );
  } else {
...
function installCodexPluginConfig(configPath, repoRoot, marketplaceName, pluginName) {
  ...
  const nextText = applyTomlSectionUpdates(
    text,
    {
      removeHeaders: ['mcp_servers."wandy"', 'mcp_servers."wandy".env'],
      upserts: [
        {
